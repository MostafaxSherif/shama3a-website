	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#SHMAعA-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#SHMAعA-contact').find('input,textarea').attr({value:''});
				jQuery('#SHMAعA-contact').css({opacity:1});
				jQuery('#SHMAعA-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#SHMAعA-contact').animate({opacity:0.3});
		jQuery('#SHMAعA-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#SHMAعA-contact').find('input,textarea').css('border','none');
